...
ga_metrics_df_weekly_2024 <- map_dfr(files_2024, ~ {
  read_csv(.x, id = "path")|>
    select(path, date = Date, page_views = Views) |>
    mutate(
      date = as_date(date, format = "%d %m %Y"),
      application = str_replace(.x, "data/google_analytics/20240611 - 2024 Pages views for test directories_", "")
    )
})
...
ga_metrics_df_weekly <- bind_rows(ga_metrics_df_weekly_2020_2022, ga_metrics_df_weekly_2023, ga_metrics_df_weekly_2024) |>
  ...
  mutate(application = case_when(application == "2020-2022_covid-19_EQA" ~ "FIND EQA directory (COVID-19)",
                                 ...
                                 application == "CXR test directory_Table" ~ "FIND Test directory (CXRs)",
                                 application == "Molecular POC test directory_Table" ~ "FIND Test directory (Molecular POC)"
  )
  ) |>
  filter(!is.na(date)) |>
  mutate(date = lubridate::as_date(date, format = "%d/%m/%Y")) |>
